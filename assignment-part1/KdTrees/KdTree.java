...
public class KdTree {
    private Node root;        // kdtree root node
    
    private class Node implements Comparable<Node> {
        private final Point2D point;          // point
        private final RectHV rect;            // rectangle
        private final int level;              // deep of the node, start from 0
        private int size;                     // size of the tree
        private Node left, right;             // left child, right child
        ...
        public int compareTo(Node parent) {                        
            if (parent.isOddLevel()) {
                if      (Double.compare(point.x(), parent.point.x()) == -1) return -1;
                else if (Double.compare(point.x(), parent.point.x()) == 1) return 1;
                else return Double.compare(point.y(), parent.point.y());
            } else {
                if      (Double.compare(point.y(), parent.point.y()) == -1) return -1;
                else if (Double.compare(point.y(), parent.point.y()) == 1) return 1;
                else return Double.compare(point.x(), parent.point.x());
            }
        }
    }
